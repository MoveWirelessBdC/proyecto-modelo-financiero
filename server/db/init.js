...
const CREATE_TABLES_SQL = `
    -- Eliminamos todo en orden para una reconstrucción limpia
    ...
    CREATE TABLE global_config ( id SERIAL PRIMARY KEY, key VARCHAR(100) UNIQUE NOT NULL, value TEXT, description TEXT );

    -- --- TABLA DE ETAPAS DEL PIPELINE ACTUALIZADA ---
    CREATE TABLE pipeline_stages (
        id SERIAL PRIMARY KEY,
        name VARCHAR(100) NOT NULL,
        stage_order INTEGER NOT NULL,
        is_archived BOOLEAN DEFAULT false, -- Para poder archivar columnas
        color VARCHAR(7) DEFAULT '#EAEAEA' -- Para el color de la columna
    );
    ...
    CREATE TABLE activities (
        id SERIAL PRIMARY KEY,
        type VARCHAR(50) NOT NULL, -- 'Note', 'Call', 'Meeting', 'Task'
    ...
    -- --- NUEVAS TABLAS PARA ETIQUETAS (TAGS) ---
    CREATE TABLE tags (
        id SERIAL PRIMARY KEY,
        name VARCHAR(50) NOT NULL,
        color VARCHAR(7) NOT NULL,
        user_id INTEGER REFERENCES users(id) -- Opcional: para saber quién creó la etiqueta
    );
...
// --- LISTA DE PERMISOS ACTUALIZADA ---
const PERMISSIONS = [
    // Permisos existentes
    'projects:view', 'projects:create', 'projects:delete', 'payments:edit',
    'clients:view', 'clients:create', 'clients:edit', 'clients:delete',
    'portfolio:view', 'portfolio:edit', 'portfolio:delete',
    'config:view', 'config:edit', 'users:view', 'users:create', 'users:delete',
    'roles:view', 'roles:edit', 'permissions:manage',
    'leads:view', 'leads:create', 'leads:edit', 'leads:delete',
    'opportunities:view:all', 'opportunities:view:team', 'opportunities:view:own',
    'opportunities:create', 'opportunities:edit', 'opportunities:delete',
    'activities:view', 'activities:create', 'activities:edit', 'activities:delete',
    // Nuevos permisos para esta fase
    'stages:edit', 'tags:manage'
];
...
const ROLE_PERMISSIONS = {
    ...
    'Admin': [...PERMISSIONS],
    'Manager': [
        'projects:view', 'projects:create', 'clients:view', 'clients:create', 'clients:edit',
        'leads:view', 'leads:create', 'leads:edit',
        'opportunities:view:team', 'opportunities:create', 'opportunities:edit',
        'activities:view', 'activities:create', 'activities:edit'
    ],
    'Team Member': [
        'projects:view', 'projects:create', 'clients:view', 'clients:create', 'clients:edit',
        'leads:view', 'leads:create', 'leads:edit',
        'opportunities:view:own', 'opportunities:create', 'opportunities:edit',
        'activities:view', 'activities:create', 'activities:edit'
    ],
    'Observer': [
        'projects:view', 'clients:view', 'portfolio:view', 'config:view', 'users:view', 'roles:view',
        'leads:view', 'opportunities:view:all', 'activities:view'
    ]
};
...
const initializeDatabase = async () => {
    ...
    try {
        console.log('--- Empezando inicialización de la base de datos (vCRM) ---');
        await client.query(CREATE_TABLES_SQL);
        console.log('1. Tablas creadas con éxito.');

        // Insertar Etapas del Pipeline por defecto
        await client.query(`
            INSERT INTO pipeline_stages (name, stage_order) VALUES
            ('Prospección', 1), ('Calificación', 2), ('Propuesta Enviada', 3), 
            ('Negociación', 4), ('Cerrado-Ganado', 5), ('Cerrado-Perdido', 6)
            ON CONFLICT (id) DO NOTHING;
        `);
        console.log('Etapas del pipeline insertadas.');
        
        const roleIds = {};
        for (const role of ROLES) {
            const res = await client.query('INSERT INTO roles (nombre_rol) VALUES ($1) ON CONFLICT (nombre_rol) DO UPDATE SET nombre_rol = EXCLUDED.nombre_rol RETURNING id', [role]);
            roleIds[role] = res.rows[0].id;
        }

        const permissionIds = {};
        for (const perm of PERMISSIONS) {
            const res = await client.query('INSERT INTO permissions (accion) VALUES ($1) ON CONFLICT (accion) DO UPDATE SET accion = EXCLUDED.accion RETURNING id', [perm]);
            permissionIds[perm] = res.rows[0].id;
        }
        console.log('2. Roles y permisos insertados.');
        ...
        for (const roleName in ROLE_PERMISSIONS) {
            ...
            for (const permName of permsToAssign) {
                if (!permissionIds[permName]) continue; // Evita errores si un permiso no existe
                const permId = permissionIds[permName];
                await client.query('INSERT INTO rol_permisos (rol_id, permiso_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [roleId, permId]);
            }
        }
        console.log('3. Permisos asignados a roles.');

        const hashedPassword = await bcrypt.hash('supersecretpassword', 10);
        const ownerRoleId = roleIds['Owner'];
        await client.query(
            'INSERT INTO users (nombre_completo, email, password_hash, rol_id) VALUES ($1, $2, $3, $4) ON CONFLICT (email) DO NOTHING',
            ['Admin User', 'admin@app.com', hashedPassword, ownerRoleId]
        );
        console.log('4. Usuario Owner creado.');

        // --- NUEVA SECCIÓN: INSERTAR ETIQUETAS DE EJEMPLO ---
        console.log('6. Insertando etiquetas de ejemplo...');
        await client.query(`
            INSERT INTO tags (name, color, user_id) VALUES
            ('Urgente', '#ef4444', 1),       -- Rojo
            ('Alto Potencial', '#3b82f6', 1), -- Azul
            ('Seguimiento', '#f97316', 1);   -- Naranja
        `);
